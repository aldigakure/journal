...
@section('content')
<div class="mb-4">
    <h2><i class="bi bi-plus-circle"></i> Tambah Jurnal Mengajar</h2>
</div>
        ...
        <form action="{{ route('journals.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="material" id="material" class="form-control @error('material') is-invalid @enderror" 
                       value="{{ old('material') }}" placeholder="Contoh: Sistem Persamaan Linear" required>
                @error('material')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" id="description" rows="3" class="form-control @error('description') is-invalid @enderror" 
                          placeholder="Jelaskan kegiatan pembelajaran yang dilakukan...">{{ old('description') }}</textarea>
                @error('description')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
                ...
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" name="student_present" id="student_present" class="form-control @error('student_present') is-invalid @enderror" 
                               value="{{ old('student_present', 0) }}" min="0" required>
                ...
                <div class="col-12 col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" name="student_absent" id="student_absent" class="form-control @error('student_absent') is-invalid @enderror" 
                               value="{{ old('student_absent', 0) }}" min="0" required>
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" id="notes" rows="2" class="form-control @error('notes') is-invalid @enderror" 
                          placeholder="Catatan khusus atau kendala yang dihadapi...">{{ old('notes') }}</textarea>
                @error('notes')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
